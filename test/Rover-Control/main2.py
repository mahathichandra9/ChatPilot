...
def main():
    ...
    try:
        obj = Bug1Navigator(path, vehicle)
        obj.run()
...
class Bug1Navigator:
    def __init__(self, waypoints, vehicle):
        self.waypoints = waypoints
        self.prev_wp_index = -1
        ...
        self.latest_servo1_value = None
        self.latest_servo3_value = None
        @self.vehicle.on_message('SERVO_OUTPUT_RAW')
        def servo_listener(self, name, message):
            self.latest_servo1_value = message.servo1_raw
            self.latest_servo3_value = message.servo3_raw
            print(f"[SERVO] Servo1: {self.latest_servo1_value}, Servo3: {self.latest_servo3_value}")
    ...
    def go_to_goal(self):
        ...
        if self.prev_wp_index != self.current_wp_index :
            target_location = LocationGlobalRelative(target_lat, target_lon, 0.0)
