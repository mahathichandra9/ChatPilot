...
def on_new_sample(sink, _):
    ...
    if asyncio_loop and clients:
        asyncio.run_coroutine_threadsafe(broadcast(frame_bytes), asyncio_loop)
...
def start_gst():
    pipeline_str = (
        ...
        "nvv4l2h264enc insert-sps-pps=true iframeinterval=1 bitrate=1000000 ! "
    ...
    pipeline.set_state(Gst.State.PLAYING)
    print("GStreamer low-latency pipeline running")
...
async def start_server():
    print("Starting WebSocket on ws://0.0.0.0:8765/video")
    async with websockets.serve(ws_handler, "0.0.0.0", 8765, max_size=None, write_limit=0, compression=None):
        await asyncio.Future()

if __name__ == "__main__":
    asyncio_loop = asyncio.get_event_loop()
    gst_thread = threading.Thread(target=start_gst, daemon=True)
    gst_thread.start()
